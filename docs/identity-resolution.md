...
The authenticated unified search also found non-structural Terpedia aliases for eucalyptol and sabinyl acetate. These are recorded in `data/terpedia-unresolved-aliases.csv` with their source record IDs and content hashes. Because the queried records expose no InChIKey or SMILES, they remain excluded from structure joins; a name-only alias is not an exact molecular identity.
