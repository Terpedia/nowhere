...
The authenticated unified search found Terpedia aliases for eucalyptol and sabinyl acetate. These are recorded in `data/terpedia-unresolved-aliases.csv` with their source record IDs and content hashes. The tabular API omitted structure fields for both aliases; the direct Eucalyptol compound page subsequently exposed an exact SMILES and InChIKey and that record was promoted into the structure table. Sabinyl acetate remains excluded because its aliases still expose no structure fields.
