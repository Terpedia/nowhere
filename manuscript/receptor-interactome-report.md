...
SAIR name and exact-SMILES searches returned no rows for the tested absinthe compounds through the public tabular API, even though the broader Terpedia KB returned HTR2A and GABA receptor target records. A read-only copy of the underlying SandboxAQ `sair.parquet` was then inspected with DuckDB: it contains 8,803,710 rows. RDKit canonicalization of seven resolved absinthe structures across 18,145 chemically compatible candidate rows found zero matches both with stereochemistry preserved and after removing stereochemistry. The two documented Terpedia raw `sair/` objects were also checked and were not present in the accessible bucket listing. This remains only a release-level join result because SAIR may contain a different structure representation, a missing compound, or an unavailable Terpedia promotion. Direct BigQuery querying also requires a role not available to the current account. Therefore, the network is a transparent first release: it includes literature-supported edges and records the missing SAIR join rather than treating missing access or missing annotation as biological absence. Results are in `data/sair-canonical-join-results.csv`; retrieval and schema metadata are in `data/sair-release-metadata.json`.
